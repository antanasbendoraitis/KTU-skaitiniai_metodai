...
% m=100
T=10;
dazniu_slenkstis=120 %m/2    % harmoniku numeriu slenkstis triuksmu filtravimui 
...
legend({'amplitudziu spektras';'dazniu slenkstis'})
...
%------------------------------------------------------------------------------------------------------
function rez=fnk(T,t), rez=(1-sign(cos(2*pi*t/T))).*sin(2*pi*3*t/T).*cos(2*pi*4*t/T)+0.15*sin(2*pi*123*t/T)+0.25*sin(2*pi*160*t/T); return, end
%function rez=fnk(T,t), rez=(1-sign(cos(2*pi*t/T))).*sin(2*pi*3*t/T).*cos(2*pi*4*t/T); return, end %be triukðmo