...
function Furje
    ...
    slenkstis=0.02 ; % harmoniku amplitudziu slenkstis triuksmu filtravimui
    dt=T/n;
    ...
    legend(sprintf('n=%d tasku, m=%d harmoniku, daznis ne daugiau 99 ',n,m))
...
function rez=fnk(T,t)
    % su triuksmais
    rez=(1-sign(sin(2.*pi.*t./T))).*cos(2.*pi.*3.*t./T) + 0.22.*sin(2.*pi.*71.*t./T)+0.11.*sin(2.*pi.*82.*t./T);
    % be triuksmu
   %rez=(1-sign(sin(2.*pi.*t/T))).*cos(2.*pi.*3.*t/T);
    return
end
